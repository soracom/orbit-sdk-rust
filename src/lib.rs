...
extern "C" {
    ...
    pub fn orbit_get_request_data(ptr: i32, len: i32) -> i32;
    pub fn orbit_get_request_data_len() -> i32;
}
...
pub fn get_request_data() -> String {
    unsafe {
        let len = orbit_get_request_data_len();
        let mut v = Vec::<u8>::with_capacity(len as usize);
        v.set_len(len as usize);
        let ptr = Box::into_raw(v.into_boxed_slice()) as *mut u8;
        let ptri = transmute::<*mut u8, i32>(ptr);
        let actual_len = orbit_get_request_data(ptri, len);
